...
std_div<- function(data, threshold=3){
  std=sd(data)
  mean=mean(data)
  z <- abs(data - mean)/std
  return(z>threshold)
...
outlierVote <- function(data, threshold = 95, mad_thresh = 3.5, std_div_thresh=3) {
  pbo <- percentile_based_outlier(data, threshold)
...
data <- matrix(training_data$debtratio, ncol = 1)
outlierVote(data)
...
# Plot outliers
sample_data <- training_data$age[sample(nrow(training_data), 5000)]
plotOutlier(sample_data)
sample_data <- training_data$SeriousDlqin2yrs[sample(nrow(training_data), 5000)]
plotOutlier(sample_data)
sample_data <- training_data$RevolvingUtilizationOfUnsecuredLines[sample(nrow(training_data), 5000)]
plotOutlier(sample_data)
sample_data <- training_data$NumberOfTime30.59DaysPastDueNotWorse[sample(nrow(training_data), 5000)]
plotOutlier(sample_data)
sample_data <- training_data$DebtRatio[sample(nrow(training_data), 5000)]
plotOutlier(sample_data)
sample_data <- training_data$MonthlyIncome[sample(nrow(training_data), 5000)]
plotOutlier(sample_data)
sample_data <- training_data$NumberOfOpenCreditLinesAndLoans[sample(nrow(training_data), 5000)]
plotOutlier(sample_data)
sample_data <- training_data$NumberOfTimes90DaysLate[sample(nrow(training_data), 5000)]
plotOutlier(sample_data)
sample_data <- training_data$NumberRealEstateLoansOrLines[sample(nrow(training_data), 5000)]
plotOutlier(sample_data)
sample_data <- training_data$NumberOfTime60.89DaysPastDueNotWorse[sample(nrow(training_data), 5000)]
plotOutlier(sample_data)
sample_data <- training_data$NumberOfDependents[sample(nrow(training_data), 5000)]
plotOutlier(sample_data)
