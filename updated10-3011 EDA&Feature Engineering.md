...
```r
#CORRELATION
(correlation_matrix <- cor(df[, 2:ncol(df)]))
```

```
## Error in cor(df[, 2:ncol(df)]): 'x' must be numeric
```
...
corrplot(cor, method = "color", addCoef.col = "black",tl.cex =0.5, cl.cex = 0.5)
```
...
monthlyincome_outlier <- outlierVote(matrix(df$monthlyincome, ncol = 1))
df <- df[!monthlyincome_outlier, ]

revolvingutilizationofunsecuredlines_outlier <- outlierVote(matrix(df$revolvingutilizationofunsecuredlines, ncol = 1))
