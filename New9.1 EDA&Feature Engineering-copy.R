...
library(readr)
df <- read_csv("~/Desktop/大四/STAT3011 Project2/STAT 3011 cs-training.csv")
...
training_data = read.csv("~/Desktop/大四/STAT3011 Project2/STAT 3011 cs-training.csv")
...
sum(total_past_due==0)
df$no_past_due <- ifelse(total_past_due==0,1,0)
df1 <- subset(df, total_past_due != 0)
total_past_due1 <- df1$numberoftime3059dayspastduenotworse + df1$numberoftime6089dayspastduenotworse + df1$numberoftimes90dayslate
df1$prop_3059 <- df1$numberoftime3059dayspastduenotworse/total_past_due1
df1$prop_6089 <- df1$numberoftime6089dayspastduenotworse/total_past_due1
df1$prop_90plus <- df1$numberoftimes90dayslate/total_past_due1
library(lattice)
library(caret)
...
train_index <- createDataPartition(df1$seriousdlqin2yrs, p = test_prop, list = FALSE)
# Split the data into training and testing sets
train_df1 <- df1[train_index, ]
test_df1 <- df1[-train_index, ]
install.packages("rpart")
library(rpart)
# Set target variable and predictor variables
model <- rpart(seriousdlqin2yrs ~ prop_3059+prop_6089+prop_90plus , data = train_df1)
# Set testing data 
...
df$prop_3059 <- ifelse(is.na(df$numberoftime3059dayspastduenotworse/total_past_due),0,df$numberoftime3059dayspastduenotworse / total_past_due)
df$prop_6089 <- ifelse(is.na(df$numberoftime6089dayspastduenotworse / total_past_due), 0, df$numberoftime6089dayspastduenotworse/total_past_due)
df$prop_90plus <- ifelse(is.na(df$numberoftimes90dayslate / total_past_due), 0, df$numberoftimes90dayslate/total_past_due)
df$category_pastdue <- ifelse(df$prop_3059 == 0 & df$prop_6089 == 0 & df$prop_90plus == 0,
                              'No Past Due',
                              ifelse(df$prop_3059 >= 0.8661,
                                     'Mostly 30-59 Days Past Due',
                                     ifelse(df$prop_90plus < 0.03846, #implies df$prop_6089>=0.09544
                                            'Mostly 30-89 Days Past Due',
                                            'Mostly Likely Past Due')))                                    
table(df$category_pastdue)

df <- df[, !(names(df) == "no_past_due")]
df <- df[, !(names(df) == "prop_3059")]
df <- df[, !(names(df) == "prop_6089")]
df <- df[, !(names(df) == "prop_90plus")]
df <- df[, !(names(df) == "numberoftime3059dayspastduenotworse")]
df <- df[, !(names(df) == "numberoftimes90dayslate")]
df <- df[, !(names(df) == "numberoftime6089dayspastduenotworse")]
